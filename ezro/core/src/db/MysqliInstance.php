...
class MysqliInstance
{
     ...
     * @throws \Exception
     */
    public function selectId($table,$idField,$id,$getInfo=array('*')){
        if(empty($getInfo)||!is_array($getInfo)){
            $getInfo=array('*');
        }
        $sql='select '.implode(',',$getInfo).' from '.$table.' where '.$idField.'=?';
        try{
            $bindType=$this->_determineType($id);
            $stmt=$this->_prepare($sql);
            $stmt->bind_param($bindType,$id);
